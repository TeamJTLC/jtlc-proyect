...
/**
 * Project data edit Dialog.
 * @author Cristian Tardivo
 */
public class InfoDialog extends JDialog implements IDialog {
     ...
     */
    public InfoDialog(JFrame parent, InfoDTO dto) {
        super(parent, Assets.getString("EDIT_PROJECT_DATA"), true);
        data = dto;
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        // Name Field
        nameField = new WebTextField(data.getName());
        ...
        // Sample date Field
        sdateField = new WebDateField(data.getSampleDate());
        ...
        // Analysis date Field
        adateField = new WebDateField(data.getAnalysisDate());
        ...
        // Dialog Icon
        setIconImage(Assets.loadImage("ic_edit"));
        // Dialog size
        setResizable(false);
        pack();
     ...
     * @return InfoDTO with changes 
     */
    @Override
    public InfoDTO getResults() {
        // If accept return dto with data and mark as changed (if necessary)
        if (result) {
            // Name
            if (!data.getName().equals(nameField.getText().trim())) {
                data.setName(nameField.getText().trim());
                data.setChanged(true);
            }
            // Description
            if (!data.getDescription().equals(descriptionText.getText().trim())) {
                data.setDescription(descriptionText.getText().trim());
                data.setChanged(true);
            }
            // Analysis date
            if (!data.getAnalysisDate().equals(adateField.getDate())) {
                data.setAnalysisDate(adateField.getDate());
                data.setChanged(true);
            }
            // Sample date
            if (!data.getSampleDate().equals(sdateField.getDate())) {
                data.setSampleDate(sdateField.getDate());
                data.setChanged(true);
            }
            return data;
        }
