 ...
 *
 * $Date: 2015-01-24 04:38:10 -0600 (Sat, 24 Jan 2015) $
 *
...
import jtlc.view.components.slider.CustomSliderUI.Thumb;
 ...
 */
public class CustomSlider<T> extends JComponent {
    ...
    public static final String AUTOADD_PROPERTY = CustomSlider.class.getName()+".auto-add";

    /** The property that controls whether the user can remove a thumb (with the delete key). */
    public static final String REMOVAL_ALLOWED = CustomSlider.class.getName()+".removal-allowed";

    // NUEVO
    public static final String MOUSE_REMOVAL_ALLOWED = CustomSlider.class.getName()+".mouse-removal-allowed";
    
    // NUEVO (Permite move el thumb desde el tick)
    public static final String TICK_MOVE_ALLOWED = CustomSlider.class.getName()+".tick-move-allowed";

    // NUEVO (Dibujar numeros al final del tick)
    public static final String PAINT_NUMBERS = CustomSlider.class.getName()+".paint-number-enabled";

    // NUEVO (Dibujar cuadro de seleccion entre 2 thumbs)
    public static final String PAINT_SQUARES = CustomSlider.class.getName()+".paint-squares-enabled";

    // NUEVO (Pintar track de recorte)
    public static final String PAINT_CUT = CustomSlider.class.getName()+".paint-cut-enabled";
    
    // NUEVO (longitud del tick)
    public static final String TICK_LENGTH = CustomSlider.class.getName()+".tick-length";
    
    // NUEVO (punto inicio del tick)
    public static final String TICK_START = CustomSlider.class.getName()+".tick-start";
        
    // NUEVO (longitud del cuadrado)
    public static final String SQUARE_LENGTH = CustomSlider.class.getName()+".square-length";
    
    // NUEVO (punto inicio del cuadrado)
    public static final String SQUARE_START = CustomSlider.class.getName()+".square-start";
    
    // NUEVO (multiplicado de la posicion para los numeros)
    public static final String MULTIPLIER_VALUE = CustomSlider.class.getName()+".multiplier-value";
    
    // NUEVO (slider linkeado a otros sliders)
    public static final String LINK_SLIDER = CustomSlider.class.getName()+".linked-slider-enalbed";
    
    // NUEVO (activa o descativa el disparo de los changelisteners)
    public static final String FIRE_CHANGE_LISTENERS = CustomSlider.class.getName()+".fire-change-listeners-enalbed";
    
    /** The property that is changed when <code>setSelectedThumb()</code> is called. */
    public static final String SELECTED_THUMB_PROPERTY = CustomSlider.class.getName()+".selected-thumb";

    /** The property that is changed when <code>setCollisionPolicy(c)</code> is called. */
    public static final String COLLISION_PROPERTY = CustomSlider.class.getName()+".collision";

    /** The property that is changed when <code>setInverted(b)</code> is called. */
    public static final String INVERTED_PROPERTY = CustomSlider.class.getName()+".inverted";

    /** The property that is changed when <code>setInverted(b)</code> is called. */
    public static final String THUMB_OVERLAP_PROPERTY = CustomSlider.class.getName()+".thumb-overlap";

    /** The property that is changed when <code>setMinimumThumbnailCount(b)</code> is called. */
    public static final String THUMB_MINIMUM_PROPERTY = CustomSlider.class.getName()+".thumb-minimum";

    /** The property that is changed when <code>setOrientation(i)</code> is called. */
    public static final String ORIENTATION_PROPERTY = CustomSlider.class.getName()+".orientation";
     ...
     * ratio.
     */
    public static final String VALUES_PROPERTY = CustomSlider.class.getName()+".values";

    /** The property that is changed when <code>setValueIsAdjusting(b)</code> is called. */
    public static final String ADJUST_PROPERTY = CustomSlider.class.getName()+".adjusting";

    /** The property that is changed when <code>setPaintTicks(b)</code> is called. */
    public static final String PAINT_TICKS_PROPERTY = CustomSlider.class.getName()+".paint ticks";
     ...
     */
    public CustomSlider(float[] thumbPositions, T[] values) {
     ...
     * @param values an array of values, each value corresponds to a value in <code>thumbPositions</code>.
     */
    public CustomSlider(int orientation, float[] thumbPositions, T[] values) {
    ...
    public CustomSliderUI<T> getUI() {
        return (CustomSliderUI<T>)ui;
    }

    // MODIFICADO
    @Override
    public void updateUI() {
        setUI(new CustomSliderAquaUI<>(this));
    }

    public void setUI(CustomSliderUI<T> ui) {
    ...
    public void removeThumb(int thumbIndex) {
        ...
        //
        removeThumbImp(thumbIndex);
        //
        if(thumbIndex % 2 == 0)
            removeThumbImp(thumbIndex);
        else
            removeThumbImp(thumbIndex - 1);
    }
     ...
     */
    private void removeThumbImp(int thumbIndex) {
        if(thumbIndex<0 || thumbIndex>thumbPositions.length)
            throw new IllegalArgumentException("There is no thumb at index "+thumbIndex+" to remove.");
    ...
    // NUEVO (agrega de a par)
    public int addThumb(float pos) {
        float a = (pos > 0.04)? pos - 0.018f:pos;
        float b = (pos < 0.94)? pos + 0.018f:pos;
        int res  = addThumbImp(a);
        res &= addThumbImp(b);
        return res;
    }
     ...
     */
    private int addThumbImp(float pos) {
        if(pos<0 || pos>1)
    ...
    // NUEVO
    public void setMouseThumbRemovalAllowed(boolean b) {
        putClientProperty(MOUSE_REMOVAL_ALLOWED, b);
    }

    // NUEVO
    public boolean isMouseThumbRemovalAllowed() {
        Boolean b = (Boolean)getClientProperty(MOUSE_REMOVAL_ALLOWED);
        if(b==null)
            b = true;
        return b;
    }
    ...
    // NUEVO
    public void setPaintNumbers(boolean val){
        putClientProperty(PAINT_NUMBERS, val);
    }

    // NUEVO
    public boolean isPaintNumbers(){
        Boolean b = (Boolean)getClientProperty(PAINT_NUMBERS);
        if(b==null) return false;
        return b;
    }

    public void setPaintSquares(boolean val){
        putClientProperty(PAINT_SQUARES, val);
    }

    // NUEVO
    public boolean isPaintSquares(){
        Boolean b = (Boolean)getClientProperty(PAINT_SQUARES);
        if(b==null) return false;
        return b;
    }

    // NUEVO
    public void setPaintCut(boolean val){
        putClientProperty(PAINT_CUT, val);
    }

    // NUEVO
    public boolean isPaintCut(){
        Boolean b = (Boolean)getClientProperty(PAINT_CUT);
        if(b==null) return false;
        return b;
    }
    ...
    public void setTickLength(int length){
        putClientProperty(TICK_LENGTH, length);
    }
    
    // NUEVO
    public int getTickLength(){
        Integer i = (Integer)getClientProperty(TICK_LENGTH);
        if(i==null) return 10;
        return i;
    }
    ...
    public void setTickStart(int start){
        putClientProperty(TICK_START, start);
    }

    // NUEVO
    public int getTickStart(){
        Integer i = (Integer)getClientProperty(TICK_START);
        if(i==null) return 0;
        return i;
    }
    ...
    public void setSquareLength(int length){
        putClientProperty(SQUARE_LENGTH, length);
    }
    
    // NUEVO
    public int getSquareLength(){
        Integer i = (Integer)getClientProperty(SQUARE_LENGTH);
        if(i==null) return 10;
        return i;
    }

    // NUEVO
    public void setSquareStart(int start){
        putClientProperty(SQUARE_START, start);
    }
    
    // Nuevo
    public int getSquareStart(){
        Integer i = (Integer)getClientProperty(SQUARE_START);
        if(i==null) return 0;
        return i;
    }
    
    // NUEVO
    public void setMultValue(float val){
        putClientProperty(MULTIPLIER_VALUE, val);
    }
    
    // Nuevo
    public float getMultValue(){
        Float i = (Float)getClientProperty(MULTIPLIER_VALUE);
        if(i==null) return 1;
        return i;
    }
    
    // NUEVO
    public void setThumbShape(Thumb shape){
        putClientProperty(CustomSliderUI.THUMB_SHAPE_PROPERTY, shape);
    }
    
    // NUEVO
    public void setTickThumbMove(boolean b){
        putClientProperty(TICK_MOVE_ALLOWED, b);
    }
    
    // NUEVO
    public boolean getTickThumbMove(){
        Boolean b = (Boolean)getClientProperty(TICK_MOVE_ALLOWED);
        if(b==null) return false;
        return b;
    }
    
    // NUEVO
    public void setLinkedStatus(boolean b){
        putClientProperty(LINK_SLIDER, b);
    }
    
    // NUEVO
    public boolean isLinkedEnabled(){
        Boolean b = (Boolean)getClientProperty(LINK_SLIDER);
        if(b==null) return false;
        return b;
    }
    
    // NUEVO
    public boolean isFireChangeListeners(){
        Boolean b = (Boolean)getClientProperty(FIRE_CHANGE_LISTENERS);
        if(b==null) return true;
        return b;
    }
    
    // NUEVO
    public void setFireChangeListeners(boolean b){
        putClientProperty(FIRE_CHANGE_LISTENERS, b);
    }
}