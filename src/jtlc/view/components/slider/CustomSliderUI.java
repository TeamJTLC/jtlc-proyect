 ...
 *
 * $Date: 2015-01-23 06:18:36 -0600 (Fri, 23 Jan 2015) $
 *
...
import jtlc.view.components.slider.CustomSlider.Collision;
//
 ...
 */
public abstract class CustomSliderUI<T> extends ComponentUI implements MouseListener, MouseMotionListener {
     ...
     * @see Thumb
     */
    public static final String THUMB_SHAPE_PROPERTY = CustomSliderUI.class.getName()+".thumbShape";
    ...
    public static enum Thumb {
        Circle() {
            ...
            public Shape getShape(float width,float height,boolean leftEdge,boolean rightEdge,boolean sharpEdgesHint) {
                return new Ellipse2D.Float(-width/2f, -height/2f, width, height);
            }
        },
        Triangle() {
            @Override
         ...
         */
        public Shape getShape(CustomSliderUI<?> sliderUI,float x,float y,int width,int height,boolean leftEdge,boolean rightEdge) {
    ...
    protected CustomSlider<T> slider;
    ...
    public CustomSliderUI(CustomSlider<T> slider) {
            this.slider = slider;
    }

    @Override
    public Dimension getMaximumSize(JComponent s) {
        CustomSlider<T> mySlider = (CustomSlider<T>)s;
        int k = Math.max( DEPTH, getPreferredComponentDepth());
        if(mySlider.getOrientation()==CustomSlider.HORIZONTAL) {
    ...
    Runnable animatingRunnable = new Runnable() {
        ...
        public void run() {
            ...
            while(!finished) {
                synchronized(CustomSliderUI.this) {
                    finished = true;
    ...
    private int getIndex(MouseEvent e) {
        ...
        if(slider.getOrientation()==CustomSlider.HORIZONTAL) {
            ...
            // AGREGADO !! Evitar Click fuera del thumb (por ej. en el tick/linea larga)
            if(!slider.getTickThumbMove()){
                w = e.getY();
                if(w<shapeSum.getMinY() || w>shapeSum.getMaxY()){
                    return -1;
                }
            }
        } else {
            ...
            if(!slider.getTickThumbMove()){
                w = e.getX();
                if(w<shapeSum.getMinX() || w>shapeSum.getMaxX()){
                    return -1;
                }
            }
        }
    ...
    public Shape getThumbShape(int thumbIndex,Point2D center) {
        ...
        return thumb.getShape(this, (float)center.getX(), (float)center.getY(), d.width, d.height, thumbIndex==0, thumbIndex==slider.getThumbCount()-1);
    }
    ...
    private void setCurrentIndicatedThumb(int i) {
        if(getProperty(slider,"CustomSlider.indicateThumb","true").equals("false")) {
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if(pressedState!=null && pressedState.selectedThumb!=-1) {
            ...
            // NUEVO PREGUNTAR POR REMOCION HABILITADA POR MOUSE
            if(outside && slider.isMouseThumbRemovalAllowed()) {
    ...
    protected boolean validatePositions(State state) {
         ...
         */
        if(p.length<=slider.getMinimumThumbnailCount() || (!slider.isMouseThumbRemovalAllowed()) ) {
            /** 
    ...
    FocusListener focusListener = new FocusListener() {
        ...
        public void focusLost(FocusEvent e) {
            ...
            //if(getProperty(slider,"CustomSlider.indicateComponent","false").toString().equals("true")) {
                slider.setSelectedThumb(-1);
            //}
            updateIndication();
            c.repaint();
    ...
    KeyListener keyListener = new KeyListener() {
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if( i!=-1 && (code==KeyEvent.VK_RIGHT || code==KeyEvent.VK_LEFT) && orientation==CustomSlider.HORIZONTAL && 
                    e.getModifiers()==Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()) {
                //insert a new thumb
                int i2;
                if((code==KeyEvent.VK_RIGHT && slider.isInverted()==false) || (code==KeyEvent.VK_LEFT && slider.isInverted()==true)) {
                    i2 = i+1;
                } else {
                    i2 = i-1;
                }
                ...
                e.consume();
                return;
            } else if( i!=-1 && (code==KeyEvent.VK_UP || code==KeyEvent.VK_DOWN) && orientation==CustomSlider.VERTICAL && 
                        e.getModifiers()==Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()) {
                //insert a new thumb
                int i2;
                if( (code==KeyEvent.VK_UP && slider.isInverted()==false) || (code==KeyEvent.VK_DOWN && slider.isInverted()==true)) {
                    i2 = i+1;
                } else {
                    i2 = i-1;
                }
                ...
                e.consume();
                return;
            } else if(code==KeyEvent.VK_DOWN && orientation==CustomSlider.HORIZONTAL && i!=-1) {
                //popup up!
            ...
            } else if(code==KeyEvent.VK_RIGHT && orientation==CustomSlider.VERTICAL && i!=-1) {
                //popup up!
    ...
    PropertyChangeListener propertyListener = new PropertyChangeListener() {
        ...
        public void propertyChange(PropertyChangeEvent e) {
            ...
            if(name.equals(CustomSlider.VALUES_PROPERTY) || name.equals(CustomSlider.ORIENTATION_PROPERTY) || name.equals(CustomSlider.INVERTED_PROPERTY)) {
                calculateGeometry();
                slider.repaint();
            } else if(name.equals(CustomSlider.SELECTED_THUMB_PROPERTY) || name.equals(CustomSlider.PAINT_TICKS_PROPERTY)) {
                slider.repaint();
            } else if(name.equals("CustomSlider.indicateComponent")) {
    ...
    protected void updateIndication() {
        synchronized(CustomSliderUI.this) {
            ...
            if(getProperty(slider,"CustomSlider.indicateComponent","false").equals("false")) {
